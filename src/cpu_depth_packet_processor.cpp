...
void CpuDepthPacketProcessor::loadP0TablesFromCommandResponse(unsigned char* buffer, size_t buffer_length)
{
    ...
    //      fwrite(impl_->p0_table0.buffer(), sizeof(uint16_t), 512*424, pFile);
    //      fwrite(impl_->p0_table1.buffer(), sizeof(uint16_t), 512*424, pFile);
    //      fwrite(impl_->p0_table2.buffer(), sizeof(uint16_t), 512*424, pFile);
    //      fclose(pFile);
    //      exit(255);
}
...
//    fclose(pFile);
    ...
    //  char currentTime_amp1[84] = "";

    //  char currentTime_a2[84] = "";
    //  char currentTime_b2[84] = "";
    //  char currentTime_amp2[84] = "";

    //  char currentTime_a3[84] = "";
    //  char currentTime_b3[84] = "";
    //  char currentTime_amp3[84] = "";


    //  sprintf(currentTime_a1, "data/%s%d.ir_a1", buffer, milli);
    //  sprintf(currentTime_b1, "data/%s%d.ir_b1", buffer, milli);
    //  sprintf(currentTime_amp1, "data/%s%d.ir_amp1", buffer, milli);

    //  sprintf(currentTime_a2, "data/%s%d.ir_a2", buffer, milli);
    //  sprintf(currentTime_b2, "data/%s%d.ir_b2", buffer, milli);
    //  sprintf(currentTime_amp2, "data/%s%d.ir_amp2", buffer, milli);

    //  sprintf(currentTime_a3, "data/%s%d.ir_a3", buffer, milli);
    //  sprintf(currentTime_b3, "data/%s%d.ir_b3", buffer, milli);
    //  sprintf(currentTime_amp3, "data/%s%d.ir_amp3", buffer, milli);

    //  FILE* pFilea1 = fopen(currentTime_a1, "wr");
    //  FILE* pFileb1 = fopen(currentTime_b1, "wr");
    //  FILE* pFileamp1 = fopen(currentTime_amp1, "wr");

    //  FILE*   pFilea2 = fopen(currentTime_a2, "wr");
    //  FILE* pFileb2 = fopen(currentTime_b2, "wr");
    //  FILE* pFileamp2 = fopen(currentTime_amp2, "wr");

    //  FILE* pFilea3 = fopen(currentTime_a3, "wr");
    //  FILE* pFileb3 = fopen(currentTime_b3, "wr");
    //  FILE* pFileamp3 = fopen(currentTime_amp3, "wr");
    ...
    for(int y = 0; y < 424; ++y)
        for(int x = 0; x < 512; ++x, m_ptr += 9)
        {
            ...
            //      fwrite(m_ptr + 0, sizeof(float), 1, pFilea1);
            //      fwrite(m_ptr + 1, sizeof(float), 1, pFileb1);
            //      fwrite(m_ptr + 2, sizeof(float), 1, pFileamp1);

            //      fwrite(m_ptr + 3, sizeof(float), 1, pFilea2);
            //      fwrite(m_ptr + 4, sizeof(float), 1, pFileb2);
            //      fwrite(m_ptr + 5, sizeof(float), 1, pFileamp2);

            //      fwrite(m_ptr + 6, sizeof(float), 1, pFilea3);
            //      fwrite(m_ptr + 7, sizeof(float), 1, pFileb3);
            //      fwrite(m_ptr + 8, sizeof(float), 1, pFileamp3);
